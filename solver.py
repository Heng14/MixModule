...
from network import U_Net,R2U_Net,AttU_Net,R2AttU_Net,MixU_Net,MixAttU_Net, MixR2U_Net, MixR2AttU_Net
import csv
...
class Solver(object):
	...
	def build_model(self):
		...
		if self.model_type =='U_Net':
			...
			self.unet = MixU_Net(img_ch=3,output_ch=1)
		elif self.model_type =='MixAttU_Net':
			self.unet = MixAttU_Net(img_ch=3,output_ch=1)
		elif self.model_type =='MixR2U_Net':
			self.unet = MixR2U_Net(img_ch=3,output_ch=1)
		elif self.model_type =='MixR2AttU_Net':
			self.unet = MixR2AttU_Net(img_ch=3,output_ch=1)
	...
	def train(self):
		...
		if os.path.isfile(unet_path):
			...
			for i, (images, GT, filename) in enumerate(self.test_loader):
				...
				imsave(f'vis/CHASEDB1/mixattunet/{filename[0]}_GT.jpg', GT.data.cpu().numpy()[0,0,...]*255)				
				imsave(f'vis/CHASEDB1/mixattunet/{filename[0]}_SR.jpg', SR.data.cpu().numpy()[0,0,...]*255)
				imsave(f'vis/CHASEDB1/mixattunet/{filename[0]}_images.jpg', images.data.cpu().numpy()[0,0,...])
				continue
